...
public class XMLMapperBuilder extends BaseBuilder {
  ...
  /**
   * 存储从 XML 配置文件中提取出来的 SQL 片段, 便于重复使用
  ...
  private void buildStatementFromContext(List<XNode> list, String requiredDatabaseId) {
    // 遍历 select delete update insert 标签
  ...
  private void cacheRefElement(XNode context) {
    if (context != null) {
      // 将当前命名空间与引用的缓存命名空间进行关联
      // builderAssistant.getCurrentNamespace() 获取当前正在解析的xml的命名空间
      // context: 当前正在解析的标签节点(cache-ref)
      configuration.addCacheRef(builderAssistant.getCurrentNamespace(), context.getStringAttribute("namespace"));
      // 创建 CacheRefResolver 对象, 解析
      CacheRefResolver cacheRefResolver = new CacheRefResolver(builderAssistant,
          context.getStringAttribute("namespace"));
      try {
        cacheRefResolver.resolveCacheRef();
      } catch (IncompleteElementException e) {
        // 解析失败, 添加到 configuration 的 incompleteCacheRefs中
  ...
  private void cacheElement(XNode context) {
    if (context != null) {
      ...
      // 获得 flushInterval size readOnly blocking 属性
  ...
  /**
   * 处理单个 resultMap 标签, 转为ResultMap对象
   */
  ...
  private ResultMap resultMapElement(XNode resultMapNode, List<ResultMapping> additionalResultMappings,
    ...
    // 获得 type 属性并获取对应的Class对象, type 制定了 java的类型, 如果type属性不存在, 尝试从其它属性获取
    String type = resultMapNode.getStringAttribute("type", resultMapNode.getStringAttribute("ofType",
        resultMapNode.getStringAttribute("resultType", resultMapNode.getStringAttribute("javaType"))));
    Class<?> typeClass = resolveClass(type);
    // 如果未获取到type类型对应的值, 尝试 继承外部类型来解析类型
    if (typeClass == null) {
    ...
    for (XNode resultChild : resultChildren) {
      ...
      if ("constructor".equals(resultChild.getName())) {
        ...
        // 构建一个ResultMapping对象加入集合
        resultMappings.add(buildResultMappingFromContext(resultChild, typeClass, flags));
      }
    }
    // 设置 id extends autoMapping 属性
    ...
    // 如果解析异常. 将该解析对象加入未完成ResultMap集合中
    try {
  ...
  private Discriminator processDiscriminatorElement(XNode context, Class<?> resultType,
    ...
    Class<? extends TypeHandler<?>> typeHandlerClass = resolveClass(typeHandler);
    JdbcType jdbcTypeEnum = resolveJdbcType(jdbcType);
    // 遍历子节点, 将 discriminator 子节点, 解析成 discriminatorMap 集合
  ...
  private void bindMapperForNamespace() {
    ...
    if (namespace != null) {
      // 获得Mapper映射配置文件对应的Mapper接口, namespace即类名
